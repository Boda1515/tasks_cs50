...
// will represent all of the preferences for voter number i[MAX_VOTERS]
// will store the index of the candidate who is the jth preference for voter i
...
bool vote(int voter, int rank, string name)
{
    ...
    for (int i = 0; i < candidate_count; i++)
    {
        ...
        {
            // voter mean: ith voter, rank mean: jth rank, i mean: the index of preference name
...
// look at all of the voters’ preferences and compute the current vote totals, by looking at each voter’s top choice candidate who
...
void tabulate(void)
{
    ...
    for (int i = 0; i < voter_count; i++)
    {
        // Find the first non-eliminated candidate in the voter's preference list
        for (int j = 0; j < candidate_count; j++)
        {
            int preference = preferences[i][j];
            if (!candidates[preference].eliminated)
            {
                // Increment the vote count of the first non-eliminated candidate whose preference matches the rank being counted
